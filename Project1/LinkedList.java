...
public class LinkedList {
	...
	public void saveFile() {
		try {
			File file = new File("fileSave.txt"); //Files object
	...
	public void linkListDelete(Student input) {
		if (headptr.next.data == input) { // check if first node is empty
			...
			while (trav1 != null) { // basic list traversal
				...
				if (trav2.data == input) { // if we locate the node with user input we will shift next pointer to next
